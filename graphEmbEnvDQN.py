...
class GraphEmbEnvDQN(gym.Env):

    def __init__(self, source_graph, target_graph, delta_heat, norm):
        super(GraphEmbEnvDQN, self).__init__()
    ...
    def step(self, action):
        ...
        if(action < len(neighbors)):
            ...
            #reward = (-200*1.5)/case_complexity
            reward = -2
    ...
    def reward_function(self, prev_avg_heat):
        #discount = self.get_avg_heat()/prev_avg_heat
        #case_complexity = self.get_case_complexity()
        reward = math.log(prev_avg_heat/self.get_avg_heat())
        return reward
    ...
    def get_avail_aux_node(self):
        if self.priority_node in self.aux_nodes:
            ...
            for node in local_aux_nodes:
                ...
                #print(f"MIN_HEAT {min_heat} MIN_HEAT_NODE {min_heat_node}")
